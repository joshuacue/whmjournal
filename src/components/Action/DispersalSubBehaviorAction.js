...
export default class DispersalSubBehaviorAction extends Component {
  constructor() {
    super();
    this.state = {
      value: '',
  ...
  pressedKey(item) {
    //User.setRecord(Bird.BIRD_NON_LETHAL, item); TODO set record dispersal
    switch (item) {
      case 'BIRD_SCARING_CARTRIDGE':
      case 'MISSED_SHOTGUN_CARTRIDGE':
      case 'MISSED_RIFLE_ROUND':
      case 'BLANK_ROUND':
        this.props.navigation.navigate(DialerClass.DISPERSAL_ACTION_NAME);
        break;
      default:
        this.props.navigation.navigate(ActionClass.DISPERSAL_AIRFIELD_NAME);
        break;
    }
    //EXAMPLE 1: Airfield -> Map / Direction -> Timer -> Dispersal Clear
    //EXAMPLE 2: Count -> Airfield -> Map / Direction -> Timer -> Dispersal Clear
  }
  pressIn() {
    timer = setInterval(() => {
      pressLen++;
  ...
  pressOut() {
    ...
    if (pressLen > 6) {
      ...
      this.props.navigation.navigate(ActionClass.DISPERSAL_BEHAVIOR_NAME);
    }
    pressLen = 0;
  }
  pressedHome() {
