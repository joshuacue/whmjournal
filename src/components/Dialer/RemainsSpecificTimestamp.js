...
export default class RemainsSpecificTimestamp extends Component {
  constructor() {
    super();
    this.state = {
      value: '',
  ...
  pressedValue() {
    if (this.state.value.length > 0) {
      //User.setRecord(Bird.BIRD_TIME, this.state.value); TODO record remains
      this.props.navigation.navigate(MapClass.REMAINS_SPECIFIC_CLEAR_NAME);
    }
  }
  pressIn() {
    timer = setInterval(() => {
  ...
  pressOut() {
    ...
    if (pressLen > 6) {
      ...
      this.props.navigation.navigate(TimerClass.REMAINS_SPECIFIC_NAME);
    }
    pressLen = 0;
  }
  pressedHome() {
