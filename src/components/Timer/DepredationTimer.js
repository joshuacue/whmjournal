...
export default class DepredationTimer extends Component {
  constructor() {
    super();
    this.state = {
      value: '',
  ...
  pressedKey(item) {
    ...
    User.setRecord(Bird.BIRD_TIME, item);
    this.props.navigation.navigate(MapClass.DEPREDATION_CLEAR_NAME);
  }
  pressIn() {
    timer = setInterval(() => {
      pressLen++;
  ...
  pressedHome() {
    ...
    else this.props.navigation.navigate(ActionClass.DISPERSAL_MENU_NAME);
  }
  pressedCustom() {
    this.props.navigation.navigate(DialerClass.DEPREDATION_TIMESTAMP_NAME);
  }
  render() {
    return (
