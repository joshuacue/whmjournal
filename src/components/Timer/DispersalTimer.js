...
export default class DispersalTimer extends Component {
  constructor() {
    super();
    this.state = {
      value: '',
  ...
  pressedKey(item) {
    ...
    //User.setRecord(Bird.BIRD_TIME, item); TODO  record dispersal
    this.props.navigation.navigate(MapClass.DISPERSAL_CLEAR_NAME);
  }
  pressIn() {
    timer = setInterval(() => {
      pressLen++;
  ...
  pressedCustom() {
    this.props.navigation.navigate(DialerClass.DISPERSAL_TIMESTAMP_NAME);
  }
  render() {
    return (
