...
export default class ListFavoriteView extends Component {
  constructor(props) {
    super(props);
    this.state = {
      navIndex: 0,
  ...
  handleBackButton() {
    this.props.navigation.navigate(ListViewClass.NAME);
    return true;
  }
  ...
  selectLetter() {
    this.props.navigation.navigate(ListViewClass.LETTER_NAME);
  }
  navLeft() {
  ...
  pressOut(item) {
    ...
    if (pressLen > 6) {
      ...
      //     }
      // }
      this.props.navigation.navigate(DialerClass.NAME);
    } else {
      this.props.navigation.navigate(DialerClass.NAME);
    }
    pressLen = 0;
