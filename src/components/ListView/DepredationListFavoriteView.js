...
export default class DepredationListFavoriteView extends Component {
  constructor(props) {
    super(props);
    this.state = {
      navIndex: 0,
  ...
  handleBackButton() {
    this.props.navigation.navigate(ListViewClass.DEPREDATION_VIEW_NAME);
    return true;
  }
  ...
  selectLetter() {
    this.props.navigation.navigate(ListViewClass.DEPREDATION_LETTER_NAME);
  }
  navLeft() {
  ...
  pressOut(item) {
    ...
    if (pressLen > 6) {
      ...
      //     }
      // }
      this.props.navigation.navigate(DialerClass.DEPREDATION_NAME);
    } else {
      User.setRecord(Bird.BIRD_RECORD, item);
      this.props.navigation.navigate(DialerClass.DEPREDATION_NAME);
    }
    pressLen = 0;
  }
